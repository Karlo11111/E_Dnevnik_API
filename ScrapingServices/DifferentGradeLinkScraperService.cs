...
namespace E_Dnevnik_API.ScrapingServices
{
    ...
    {
        public async Task<List<GradeSubjectDetails>> ScrapeDifferentGradeLink(HttpClient client)
        {
            // prvo dohvaćamo listu svih razreda s /class stranice
            var gradeLinks = await FetchGradeLinks(client);
            ...
            foreach (var grade in gradeLinks.DifferentGrade)
            {
                ...
                var classPageResponse = await client.GetAsync(classPageUrl);
                if (!classPageResponse.IsSuccessStatusCode)
                    continue;

                var classPageContent = await classPageResponse.Content.ReadAsStringAsync();

                var subjectDetails = await ExtractSubjectDetails(classPageContent, client);
                allGradeSubjectDetails.Add(
                    new GradeSubjectDetails(grade.GradeYear, subjectDetails.Subjects)
                );
            }
        ...
        private async Task<DifferentGradeResult> FetchGradeLinks(HttpClient httpClient)
        {
            ...
            return ExtractGradeLinks(htmlContent);
        }

        private DifferentGradeResult ExtractGradeLinks(string htmlContent)
        {
        ...
        private async Task<SubjectScrapeResult> ExtractSubjectDetails(
            string htmlContent,
